...
namespace OCI {
  const std::map< std::string, 
          std::function<
            std::unique_ptr< OCI::Base::Client >( std::string const&, std::string const&, std::string const& )
          >
        > CLIENT_MAP = {
    { "dir",
      [](std::string const& base_dir, [[maybe_unused]] std::string const& username, [[maybe_unused]] std::string const& password ) {
