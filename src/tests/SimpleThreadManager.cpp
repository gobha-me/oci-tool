...
//  }
  ...
  SECTION( "Background Breaker" ) {
    ...
    for (; i != 8; i++ ) {
      ep_outter.background( [ &stm, &value, i ]() {
        auto ep_inner = stm.executionPool( "Test Case Background Breaker Inner " + std::to_string( i ) );

        ep_inner.execute( [ &value, i ]() {
          spdlog::info( "Background Value add Inner execution {}", i );
          value++;
        } );
